...
class Gan:
    ...
    def __init__(self):
        self.discriminator_input = tf.placeholder(dtype=tf.float32, shape=[None, 28, 28], name='d_input')
    ...
    def build_loss(self):
        ...
        self.loss_d_real = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(self.discriminator_real),
                                                              logits=self.discriminator_real)
        print(tf.shape(tf.ones_like(self.discriminator_real)))
        self.loss_d_fake = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(self.discriminator_fake),
                                                              logits=self.discriminator_fake)
        self.loss_g = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(self.discriminator_fake),
                                                                        logits= self.discriminator_fake))
        ...
        with tf.control_dependencies(update_ops):
            self.optimizer_d = tf.train.AdamOptimizer(0.0002).minimize(self.loss_d + d_reg,
                                                                                         var_list=vars_d)
            self.optimizer_g = tf.train.AdamOptimizer(0.0002).minimize(self.loss_g + g_reg,
                                                                                         var_list=vars_g)
